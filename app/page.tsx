...
export default async function Home(props: Props) {
  ...
  let links: any[] = []
  try {
    links = await prisma.link.findMany({ where: whereCondition, orderBy: { createdAt: 'desc' } })
    ...
    <div className="flex min-h-screen bg-[#0f172a] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-sky-900/20 via-[#0f172a] to-[#0f172a] text-slate-300 font-sans selection:bg-sky-500/30">
      ...
      <aside className="w-64 border-r border-slate-800/60 bg-slate-900/40 backdrop-blur-xl flex-col hidden md:flex fixed h-full z-20">
        ...
        <nav className="flex-1 overflow-y-auto px-4 space-y-1.5 custom-scrollbar">
          <a href="/?category=All" className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${currentCategory === 'All' ? 'bg-slate-800 text-white ring-1 ring-slate-700' : 'hover:bg-slate-800/50 hover:text-white text-slate-400'}`}>
            <span>全部工具</span><span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded-md">All</span>
          </a>
          <a href="/?category=Recommended" className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${currentCategory === 'Recommended' ? 'bg-sky-600/90 text-white shadow-lg shadow-sky-500/20 ring-1 ring-sky-500' : 'hover:bg-slate-800/50 hover:text-white text-slate-400'}`}>
          ...
          {categoriesData.map((cat) => (
            <a key={cat.category} href={`/?category=${cat.category}`} className={`flex items-center justify-between px-4 py-2.5 rounded-xl text-sm transition-all duration-200 group ${currentCategory === cat.category ? 'bg-slate-800 text-sky-400 ring-1 ring-slate-700' : 'hover:bg-slate-800/50 hover:text-white text-slate-400'}`}>
      ...
      <main className="flex-1 md:ml-64 p-6 md:p-10 relative z-10">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
                <h2 className="text-3xl font-bold text-white mb-2 tracking-tight">{currentCategory === 'All' ? '全部工具' : (currentCategory === 'Recommended' ? '站长推荐' : currentCategory)}</h2>
                <p className="text-slate-400 text-sm flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-sky-500"></span>{searchQuery ? `搜索 "${searchQuery}" 的结果` : `收录了 ${links.length} 个优质资源`}</p>
            </div>
